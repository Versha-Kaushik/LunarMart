...
const categoryData = {
  'wall-art': { name: 'Wall Art' },
  'vases': { name: 'Vases' },
  'scented-candles': { name: 'Scented Candles' },
...
const CategoryDetailPage = () => {
  ...
  if (!category) {
    return (
      <div className='text-center py-5'>
        ...
        <Button variant='primary' onClick={() => navigate('/categories')} className='mt-3'>
  ...
  return (
    <>
      <Meta title={`${category.name} - LunarMart`} />
      <div style={{ marginTop: '40px', position: 'relative', textAlign: 'center' }}>
        ...
        <h1 style={{ color: '#e2e8f0', margin: '0 40px' }}>{category.name}</h1>
      </div>
      ...
      <div className='my-4'>
        {isLoading ? (
          ...
          <div className='text-center py-5'>
            <h4 style={{ color: '#94a3b8' }}>No products found in this category.</h4>
            <p className='text-secondary'>Please check back later or explore other categories!</p>
          </div>
        ) : (
          <Row className='g-4'>
            {allItems.map(item => (
